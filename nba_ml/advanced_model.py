...
PLAYER_EMBED_DIM = 40
TEAM_REPR_DIM = 112
MATCHUP_DIM = 160  # matchup interaction output
HIDDEN_DIM = 192
NUM_ATTN_HEADS = 2
NUM_TRUNK_LAYERS = 5
DROPOUT = 0.3462072844310212
...
# Training per model
BATCH_SIZE = 256
LEARNING_RATE = 0.0001096524277832185
WEIGHT_DECAY = 1.4853153980421619e-05
WARMUP_STEPS = 200
MAX_EPOCHS = 40
EARLY_STOP_PATIENCE = 8
LABEL_SMOOTHING = 0.12150897038028768
LOSS_W_WIN = 0.5637147078172364
LOSS_W_MARGIN = 1.0421378884426276
LOSS_W_TOTAL = 0.38011437030470097
...
def _derive_features(raw):
    ...
    return np.array(
        [
            ...
            pts / max(mins, 1.0),
            ast / max(tov, 0.5),
            (pts + reb + ast + stl + blk - tov) / max(mins, 1.0),
        ],
        dtype=np.float32,
    )
...
class PlayerStatTracker:
    ...
    def update(self, player_name, game_stats):
        ...
        if n == 0:
            ...
            self.short[player_name] = (1 - self.alpha_short) * self.short[
                player_name
            ] + self.alpha_short * game_stats
            self.long[player_name] = (1 - self.alpha_long) * self.long[
                player_name
            ] + self.alpha_long * game_stats
    ...
    def get_team_roster(self, team_players):
        ...
        for i, (pn, _) in enumerate(
            sorted(team_players, key=lambda x: x[1][6], reverse=True)[:PLAYERS_PER_TEAM]
        ):
            roster[i] = self.get(pn)
            mask[i] = 1.0
        return roster, mask

    def update_team(self, team_players):
        for pn, st in team_players:
            self.update(pn, st)
    ...
    def load(cls, path):
        with open(path, "rb") as f:
            d = pickle.load(f)
        t = cls(
            d.get("as", d.get("alpha_short", 0.25)),
            d.get("al", d.get("alpha_long", 0.06)),
        )
        t.short = d.get("s", d.get("short", d.get("stats", {})))
        t.long = d.get("l", d.get("long", d.get("stats", {})))
        t.games = d.get("g", d.get("games", {}))
        return t
...
class TeamStatTracker:
    ...
    def update(self, team, won, margin, total, is_home):
        ...
        if is_home:
            t["hw"] = (1 - self.alpha_short) * t["hw"] + self.alpha_short * w
        t["str"] = max(t["str"], 0) + 1 if won else min(t["str"], 0) - 1
        t["gp"] += 1
    ...
    def save(self, path):
        with open(path, "wb") as f:
            pickle.dump(
                {"as": self.alpha_short, "al": self.alpha_long, "t": self.teams}, f
            )
    ...
    def load(cls, path):
        with open(path, "rb") as f:
            d = pickle.load(f)
        t = cls(d.get("as", 0.15), d.get("al", 0.04))
        t.teams = d.get("t", d.get("teams", {}))
        return t
...
class EloTracker:
    ...
    def expected(self, home, away):
        return 1.0 / (
            1.0 + 10.0 ** (-(self.get(home) - self.get(away) + self.home_adv) / 400.0)
        )
    ...
    def reset_season(self):
        for t in list(self.ratings):
            self.ratings[t] = self.ratings[t] * 0.75 + 375.0

    def save(self, path):
        with open(path, "wb") as f:
            pickle.dump({"k": self.k, "ha": self.home_adv, "r": self.ratings}, f)
...
class RestTracker:
    ...
    def update(self, team, game_date):
        self.last_game[team] = game_date

    def save(self, path):
        with open(path, "wb") as f:
            pickle.dump(self.last_game, f)

    @classmethod
    def load(cls, path):
        with open(path, "rb") as f:
            r = cls()
            r.last_game = pickle.load(f)
            return r
...
class PlayerEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(PLAYER_INPUT_DIM, PLAYER_EMBED_DIM),
            nn.GELU(),
            nn.LayerNorm(PLAYER_EMBED_DIM),
...
class RosterAggregator(nn.Module):
    def __init__(self):
        ...
        self.attn1 = nn.MultiheadAttention(
            PLAYER_EMBED_DIM, NUM_ATTN_HEADS, dropout=DROPOUT * 0.5, batch_first=True
        )
        ...
        self.proj = nn.Sequential(
            ...
            nn.GELU(),
            nn.LayerNorm(TEAM_REPR_DIM),
        )

    def forward(self, embeds, mask):
        pad = mask == 0
        x = self.norm1(embeds)
        a1, _ = self.attn1(x, x, x, key_padding_mask=pad)
        x = embeds + a1
        x2 = self.norm2(x)
        a2, _ = self.attn2(x2, x2, x2, key_padding_mask=pad)
        x = x + a2
...
class MatchupInteraction(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(TEAM_REPR_DIM * 3, MATCHUP_DIM * 2),
            nn.GELU(),
            nn.LayerNorm(MATCHUP_DIM * 2),
            nn.Dropout(DROPOUT),
            nn.Linear(MATCHUP_DIM * 2, MATCHUP_DIM),
            nn.GELU(),
        )
...
class SequentialNBAModel(nn.Module):
    def __init__(self):
        ...
        self.trunk = nn.Sequential(
            *[ResidualBlock(HIDDEN_DIM) for _ in range(NUM_TRUNK_LAYERS)]
        )
    ...
    def forward(self, hr, hm, ar, am, htf, atf, ctx):
        h_emb = self.player_encoder(hr)
        a_emb = self.player_encoder(ar)
        h_repr = self.roster_aggregator(h_emb, hm)
        a_repr = self.roster_aggregator(a_emb, am)
        ...
        return (
            self.win_head(x).squeeze(-1),
            self.margin_head(x).squeeze(-1),
            self.total_head(x).squeeze(-1),
        )
...
def _collect_features(games, game_index, emit, progress_state, cfg=None):
    _cfg = cfg or {}
    player_tracker = PlayerStatTracker(
        alpha_short=_cfg.get("player_alpha_short", 0.25),
        alpha_long=_cfg.get("player_alpha_long", 0.06),
    )
    team_tracker = TeamStatTracker(
        alpha_short=_cfg.get("team_alpha_short", 0.15),
        alpha_long=_cfg.get("team_alpha_long", 0.04),
    )
    elo_tracker = EloTracker(
        k=_cfg.get("elo_k", 20),
        home_adv=_cfg.get("elo_home_adv", 100),
    )
    rest_tracker = RestTracker()

    _sw_year = _cfg.get("sample_weight_year", 2010)
    _sw_slope = _cfg.get("sample_weight_slope", 0.08)
    ...
    for idx, (_, row) in enumerate(games.iterrows()):
        ...
        if prev_season is not None and season != prev_season:
            team_tracker.reset_season()
            elo_tracker.reset_season()
        prev_season = season

        gp = game_index.get(gid, {})
        hp = gp.get(ht, [])
        ap = gp.get(at, [])
        if not hp or not ap:
            continue
        ...
        htf = team_tracker.get(ht)
        atf = team_tracker.get(at)

        gd = row["date"]
        sd = (gd - pd.Timestamp(f"{season - 1}-10-01")).days
        sp = min(1.0, max(0.0, sd / 250.0))
        elo_diff = (elo_tracker.get(ht) - elo_tracker.get(at)) / 400.0
        elo_exp = elo_tracker.expected(ht, at)
        hr_rest = rest_tracker.get_rest(ht, gd) / 7.0
        ar_rest = rest_tracker.get_rest(at, gd) / 7.0
        hb2b = float(rest_tracker.get_rest(ht, gd) <= 1.0)
        ab2b = float(rest_tracker.get_rest(at, gd) <= 1.0)
        ...
        mg = float(row["home_margin"])
        tt = float(row["total_score"])
        weight = 1.0 + max(0.0, (season - _sw_year)) * _sw_slope

        all_hr.append(home_roster)
        all_hm.append(home_mask)
        all_ar.append(away_roster)
        all_am.append(away_mask)
        all_htf.append(htf)
        all_atf.append(atf)
        all_ctx.append(ctx)
        all_win.append(hw)
        all_margin.append((mg - margin_mean) / margin_std)
        all_total.append((tt - total_mean) / total_std)
        all_weights.append(weight)

        player_tracker.update_team(hp)
        player_tracker.update_team(ap)
        team_tracker.update(ht, hw > 0.5, mg, tt, True)
        team_tracker.update(at, hw < 0.5, -mg, tt, False)
        elo_tracker.update(ht, at, hw > 0.5, mg)
        rest_tracker.update(ht, gd)
        rest_tracker.update(at, gd)

        if (idx + 1) % 3000 == 0:
            el = time.time() - t0
            spd = (idx + 1) / max(el, 0.1)
            eta = (total_games - idx - 1) / max(spd, 0.1)
            emit(
                f"  [COLLECT] {idx + 1:,}/{total_games:,} ({(idx + 1) / total_games:.0%}) | {spd:.0f} g/s | ETA: {int(eta)}s"
            )
                ...
                progress_state.update(
                    {
                        ...
                        "pct": round((idx + 1) / total_games * 20, 1),
                        "phase": "COLLECT",
                        "eta_display": f"{int(eta // 60)}m {int(eta % 60)}s",
                        "games_per_sec": round(spd, 1),
                    }
                )
    ...
    dataset = {
        ...
        "ar": np.array(all_ar),
        "am": np.array(all_am),
        "htf": np.array(all_htf),
        "atf": np.array(all_atf),
        "ctx": np.array(all_ctx),
        "win": np.array(all_win, dtype=np.float32),
        "margin": np.array(all_margin, dtype=np.float32),
        "total": np.array(all_total, dtype=np.float32),
...
def _train_single_model(dataset, split, device, seed, model_idx, emit, progress_state):
    ...
    def to_t(key):
        return torch.FloatTensor(dataset[key])
    ...
    sched = WarmupCosineScheduler(
        optimizer, WARMUP_STEPS, (split // BATCH_SIZE) * MAX_EPOCHS
    )
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for i in range(0, len(perm), BATCH_SIZE):
            ...
            optimizer.zero_grad()
            wl, mp, tp = model(
                b["hr"], b["hm"], b["ar"], b["am"], b["htf"], b["atf"], b["ctx"]
            )
            smooth = b["w"] * (1 - LABEL_SMOOTHING) + 0.5 * LABEL_SMOOTHING
            loss = (
                LOSS_W_WIN
                * F.binary_cross_entropy_with_logits(wl, smooth, weight=b["wt"])
        ...
        model.eval()
        orig_tr_idx = torch.arange(
            split // 2
        )  # original (non-augmented) training games
        tr_correct = 0
        tc = 0
        with torch.no_grad():
            for i in range(0, len(orig_tr_idx), BATCH_SIZE * 2):
                ...
                tr_correct += (
                    ((torch.sigmoid(wl) > 0.5).float() == tgt_w[bi].to(device))
                    .sum()
                    .item()
                )
        ...
        emit(
            f"    [{model_idx + 1}/{N_ENSEMBLE}] Ep {epoch + 1:2d}/{MAX_EPOCHS} | Loss: {e_loss / max(batches, 1):.4f} "
        ...
        if progress_state is not None:
            ...
            progress_state.update(
                {
                    "pct": round(base_pct + ep_pct, 1),
                    "phase": "TRAIN",
                    "train_acc": round(tr_acc * 100, 1),
                    "test_acc": round(t_acc * 100, 1),
        ...
        if t_acc > best_acc:
            ...
            if patience >= EARLY_STOP_PATIENCE:
                emit(
                    f"    [{model_idx + 1}/{N_ENSEMBLE}] Early stop ep {epoch + 1} (best: {best_acc:.1%})"
                )
                break
...
def _train_network(dataset_raw, device, emit, progress_state, cfg=None):
    """Train ensemble of N models on augmented data."""
    _cfg = cfg or {}
    n_raw = len(dataset_raw["win"])
    split_raw = int(n_raw * _cfg.get("train_split", 0.85))
    ...
    dataset = {
        k: np.concatenate([augmented_train[k], test_slice[k]], axis=0)
        for k in dataset_raw
    }
    split = n_aug  # new split point

    emit(
        f"  Total: {len(dataset['win']):,} | Train: {split:,} | Test: {len(dataset['win']) - split:,}"
    )

    model_count = sum(
        p.numel() for p in SequentialNBAModel().parameters() if p.requires_grad
    )
    emit(
        f"  Model: {model_count:,} params × {N_ENSEMBLE} ensemble = {model_count * N_ENSEMBLE:,} total"
    )
    ...
    for mi, seed in enumerate(ENSEMBLE_SEEDS):
        ...
        state, acc = _train_single_model(
            dataset, split, device, seed, mi, emit, progress_state
        )
    ...
    hr_t = torch.FloatTensor(dataset["hr"])
    hm_t = torch.FloatTensor(dataset["hm"])
    ar_t = torch.FloatTensor(dataset["ar"])
    am_t = torch.FloatTensor(dataset["am"])
    htf_t = torch.FloatTensor(dataset["htf"])
    atf_t = torch.FloatTensor(dataset["atf"])
    ctx_t = torch.FloatTensor(dataset["ctx"])
    win_t = torch.FloatTensor(dataset["win"])
...
def train_model(progress_callback=None, progress_state=None):
    ...
    player_tracker.save(PLAYER_STATS_PATH)
    team_tracker.save(TEAM_STATS_PATH)
    elo_tracker.save(ELO_PATH)
    rest_tracker.save(REST_PATH)
...
def get_model_status():
    ...
    try:
        ...
        games["date"] = pd.to_datetime(games["date"])
        last = games["date"].max()
        status["last_data_date"] = last.strftime("%Y-%m-%d")
        yesterday = pd.Timestamp(date.today()) - pd.Timedelta(days=1)
        status["data_status"] = (
            "current"
            if last >= yesterday
            else "slightly_behind"
            if last >= yesterday - pd.Timedelta(days=3)
            else "behind"
        )
    except Exception:
...
def learn_from_results(target_date=None, progress_callback=None):
    ...
    player_tracker = (
        PlayerStatTracker.load(PLAYER_STATS_PATH)
        if PLAYER_STATS_PATH.exists()
        else PlayerStatTracker()
    )
    team_tracker = (
        TeamStatTracker.load(TEAM_STATS_PATH)
        if TEAM_STATS_PATH.exists()
        else TeamStatTracker()
    )
    ...
    for event in espn_data.get("events", []):
        ...
        if not ht or not at:
            continue
        hp = _get_recent_players(logs, ht)
        ap = _get_recent_players(logs, at)
        if not hp or not ap:
            continue
        hr, hmask = player_tracker.get_team_roster(hp)
        ar, amask = player_tracker.get_team_roster(ap)
        htf = team_tracker.get(ht)
        atf = team_tracker.get(at)
        elo_diff = (elo_tracker.get(ht) - elo_tracker.get(at)) / 400.0
        elo_exp = elo_tracker.expected(ht, at)
        ...
        games_data.append(
            {
                ...
                "ctx": ctx,
                "win": hw,
                "m_norm": (mg - norm_params["margin_mean"]) / norm_params["margin_std"],
    ...
    if CACHED_ODDS_PATH.exists():
        try:
            with open(CACHED_ODDS_PATH) as f:
    ...
    for i, g in enumerate(games_data):
        ...
        with torch.no_grad():
            for m in models:
                m.eval()
                wl, mp, tp = m(
                    torch.FloatTensor(g["hr"]).unsqueeze(0).to(device),
                ...
                avg_mg += (
                    mp.item() * norm_params["margin_std"] + norm_params["margin_mean"]
                ) / len(models)
                avg_tt += (
                    tp.item() * norm_params["total_std"] + norm_params["total_mean"]
                ) / len(models)
        ...
        if book_home_ml is not None:
            ...
            book_fav_home = (
                book_home_ml < 0 if (book_home_ml and book_away_ml) else None
            )
            model_fav_home = avg_wp > 0.5
            # Did our model's pick (fav) win?
            model_pick_won = (model_fav_home and home_won) or (
                not model_fav_home and not home_won
            )
        ...
        results.append(
            {
                ...
                "pred_margin": round(avg_mg, 1),
                "pred_total": round(avg_tt, 1),
                "actual_margin": round(actual_margin, 1),
                "actual_total": round(actual_total, 1),
                # Model accuracy
                "ml_correct": bool(ml_hit),
                "spread_correct": bool(spread_hit),
                "ou_result": ou_result,
    ...
    emit(
        f"Pre-learn — ML: {ml_acc_before:.0%} ({ml_correct_before}/{n}), "
        f"Spread: {spread_acc_before:.0%} ({spread_correct_before}/{n})"
    )
    if games_with_odds:
        emit(
            f"Model vs Books — Spread: {spread_vs_book}/{games_with_odds}, "
    ...
    for mi, m in enumerate(models):
        ...
        for ep in range(5):
            ...
            loss = (
                F.binary_cross_entropy_with_logits(wl, t_w)
                + 0.5 * huber(mp, t_m)
                + 0.3 * huber(tp, t_t)
            )
            loss.backward()
    ...
    for i, g in enumerate(games_data):
        ...
        if (avg_wp > 0.5) == (g["win"] > 0.5):
            ml_correct_after += 1
        if (avg_mg_post > 0) == (g["margin"] > 0):
            spread_correct_after += 1

    ml_acc_after = ml_correct_after / n
    spread_acc_after = spread_correct_after / n
    emit(
        f"Post-learn — ML: {ml_acc_after:.0%} ({ml_correct_after}/{n}), "
        f"Spread: {spread_acc_after:.0%} ({spread_correct_after}/{n})"
    )

    # Update trackers
    for g in games_data:
        player_tracker.update_team(g["home_players"])
        player_tracker.update_team(g["away_players"])
        team_tracker.update(
            g["home_team"], g["win"] > 0.5, g["margin"], g["total"], True
        )
        team_tracker.update(
            g["away_team"], g["win"] < 0.5, -g["margin"], g["total"], False
        )
    ...
    entry = {
        ...
        "games": n,
        "ml_acc_before": round(ml_acc_before, 4),
        "ml_acc_after": round(ml_acc_after, 4),
        "spread_acc_before": round(spread_acc_before, 4),
        "spread_acc_after": round(spread_acc_after, 4),
    }
    if book_stats:
    ...
    return {
        "date": target_date,
        "games": n,
        "ml_acc_before": round(ml_acc_before * 100, 1),
        "ml_acc_after": round(ml_acc_after * 100, 1),
        "spread_acc_before": round(spread_acc_before * 100, 1),
        "spread_acc_after": round(spread_acc_after * 100, 1),
...
def predict_today(model=None, meta=None):
    ...
    # Parse ESPN odds from the scoreboard data (free, no API key needed).
    # ESPN embeds DraftKings lines directly in the scoreboard response.
    def _espn_num(s):
        """Parse strings like '-1.5', '+105', 'o222.5' into floats."""
        if s is None:
            return None
        try:
    ...
    espn_odds_map = {}
    for _ev in espn_data.get("events", []):
        _comp = _ev["competitions"][0]
        _ht = _at = None
        for _c in _comp["competitors"]:
            _abbr = _csv_abbr(_c["team"]["abbreviation"])
            if _c["homeAway"] == "home":
                _ht = _abbr
            else:
                _at = _abbr
        if not _ht or not _at:
            continue
        _odds_list = _comp.get("odds", [])
        if not _odds_list:
            continue
        _o = _odds_list[0]
        _ml = _o.get("moneyline", {})
        _ps = _o.get("pointSpread", {})
        _tot = _o.get("total", {})
        _home_ml = _espn_num(_ml.get("home", {}).get("close", {}).get("odds"))
        _away_ml = _espn_num(_ml.get("away", {}).get("close", {}).get("odds"))
        # Use home spread directly (e.g. "-1.5" means home favored by 1.5)
        _home_spread = _espn_num(_ps.get("home", {}).get("close", {}).get("line"))
        _total = _espn_num(_tot.get("over", {}).get("close", {}).get("line"))
        if any(v is not None for v in [_home_ml, _away_ml, _home_spread, _total]):
            espn_odds_map[(_ht, _at)] = {
                "home": _ht, "away": _at,
                "spread": _home_spread,
                "total": _total,
                "home_ml": int(_home_ml) if _home_ml is not None else None,
                "away_ml": int(_away_ml) if _away_ml is not None else None,
            }
    ...
    for event in espn_data.get("events", []):
        ...
        for c in comp["competitors"]:
            abbr = _csv_abbr(c["team"]["abbreviation"])
            if c["homeAway"] == "home":
                ht, hd = abbr, c["team"]["displayName"]
                h_logo = c["team"].get("logo", "")
            else:
                at, ad = abbr, c["team"]["displayName"]
                a_logo = c["team"].get("logo", "")
        if not ht or not at:
            continue
        ...
        home_out = _get_out_players(injuries.get(ht, []))
        away_out = _get_out_players(injuries.get(at, []))
        home_roster, home_mask = player_tracker.get_team_roster(hp)
        away_roster, away_mask = player_tracker.get_team_roster(ap)
        hm_count = am_count = 0

        for i, (pn, _) in enumerate(
            sorted(hp, key=lambda x: x[1][6], reverse=True)[:PLAYERS_PER_TEAM]
        ):
            npn = (
                unicodedata.normalize("NFD", pn)
                .encode("ascii", "ignore")
                .decode("ascii")
            )
            if pn in home_out or npn in home_out:
                home_mask[i] = 0.0
                hm_count += 1
        for i, (pn, _) in enumerate(
            sorted(ap, key=lambda x: x[1][6], reverse=True)[:PLAYERS_PER_TEAM]
        ):
            npn = (
                unicodedata.normalize("NFD", pn)
                .encode("ascii", "ignore")
            ...
            if pn in away_out or npn in away_out:
                away_mask[i] = 0.0
                am_count += 1
        ...
        elo_diff = (elo_tracker.get(ht) - elo_tracker.get(at)) / 400.0
        elo_exp = elo_tracker.expected(ht, at)
        h_rest = rest_tracker.get_rest(ht, today_date) / 7.0
        a_rest = rest_tracker.get_rest(at, today_date) / 7.0
        hb2b = float(rest_tracker.get_rest(ht, today_date) <= 1.0)
        ab2b = float(rest_tracker.get_rest(at, today_date) <= 1.0)
        ctx = np.array(
            [
                1.0,
                0.7,
        ...
        with torch.no_grad():
            ...
            for m in models:
                ...
                avg_mg += (
                    -(
                        mp2.item() * norm_params["margin_std"]
                        + norm_params["margin_mean"]
                    )
                    / n_preds
                )  # flip margin
                avg_tt += (
                    tp2.item() * norm_params["total_std"] + norm_params["total_mean"]
                ) / n_preds

        _api_odds = odds_map.get((ht, at), {})
        # Fall back to ESPN odds if the paid API has no lines for this game
        _has_lines = any(_api_odds.get(k) is not None for k in ("spread", "total", "home_ml", "away_ml"))
        odds = _api_odds if _has_lines else espn_odds_map.get((ht, at), {})
        ...
        pred = {
            ...
            "home_logo": h_logo,
            "away_logo": a_logo,
            "win_prob": round(avg_wp * 100, 1),
            "predicted_margin": round(avg_mg, 1),
            "predicted_total": round(avg_tt, 1),
            "confidence": confidence,
            "odds_spread": odds.get("spread"),
            "odds_total": odds.get("total"),
            "odds_home_ml": odds.get("home_ml"),
            "odds_away_ml": odds.get("away_ml"),
            "home_missing": hm_count,
            "away_missing": am_count,
        }

        picks = []
        fav = ht if avg_wp > 0.5 else at
        if fav_prob >= 0.55:
            picks.append(
                {
                    "type": "ML",
        ...
        if odds.get("spread") is not None:
            # edge = margin + homeSpread  (homeSpread negative when home is favoured)
            ...
            if abs(edge) > 2.0:
                pt = (
                    f"{ht} {odds['spread']:+.1f}"
                    if edge > 0
                    else f"{at} {-odds['spread']:+.1f}"
                )
                picks.append(
                    {
                        "type": "Spread",
                        "pick": pt,
                        "confidence": round(min(80, 52 + abs(edge) * 2.5), 1),
                        "edge": round(edge, 1),
                    }
                )
        if odds.get("total") is not None:
            ...
            if abs(te) > 4:
                picks.append(
                    {
                        "type": "Total",
                        "pick": f"{'Over' if te > 0 else 'Under'} {odds['total']}",
                        "confidence": round(min(75, 52 + abs(te) * 1.5), 1),
                        "edge": round(te, 1),
                    }
                )
        pred["picks"] = picks
    ...
    for p in predictions:
        ...
        cache[today][key] = {
            "spread": p.get("odds_spread"),
            "total": p.get("odds_total"),
            "home_ml": p.get("odds_home_ml"),
            "away_ml": p.get("odds_away_ml"),
            "pred_margin": p["predicted_margin"],
        ...
        odds_rows.append(
            {
                ...
                "book_spread": p.get("odds_spread"),
                "book_total": p.get("odds_total"),
                "book_home_ml": p.get("odds_home_ml"),
                "book_away_ml": p.get("odds_away_ml"),
                "pred_margin": p["predicted_margin"],
                "pred_total": p["predicted_total"],
                "pred_win_prob": p["win_prob"],
            }
    ...
    if len(dates) > 14:
        for d in dates[:-14]:
            del cache[d]
    with open(CACHED_ODDS_PATH, "w") as f:
...
def main():
    ...
    if args.status:
        ...
        print(json.dumps(learn_from_results(), indent=2))
    else:
        train_model()
