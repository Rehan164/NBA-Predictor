...
def format_predictions_for_claude(
    game_predictions: List[Dict],
    odds: Optional[List[Dict]] = None,
    player_props: Optional[Dict] = None,
) -> str:
    """
    Format all predictions into a comprehensive text block for Claude analysis.
    """
    lines = ["=" * 70]
    lines.append("MODEL PREDICTIONS FOR TODAY'S GAMES")
    lines.append("=" * 70)

    for i, game in enumerate(game_predictions):
        if "error" in game:
            lines.append(f"\n{game.get('away_team', '?')} @ {game.get('home_team', '?')}: {game['error']}")
            continue

        home = game["home_team"]
        away = game["away_team"]
        preds = game["predictions"]

        lines.append(f"\n{'─' * 50}")
        lines.append(f"GAME {i+1}: {away} @ {home}")
        lines.append(f"{'─' * 50}")

        # Odds comparison if available
        if odds:
            game_odds = next((o for o in odds if o.get("home_team", "").endswith(home) or home in o.get("home_team", "")), None)
            if game_odds:
                lines.append(f"  Market Lines:")
                if game_odds.get("spread") is not None:
                    lines.append(f"    Spread: {home} {game_odds['spread']:+.1f} ({game_odds.get('spread_odds_home', -110)})")
                if game_odds.get("total") is not None:
                    lines.append(f"    Total: {game_odds['total']} (O {game_odds.get('over_odds', -110)} / U {game_odds.get('under_odds', -110)})")
                if game_odds.get("home_ml") is not None:
                    lines.append(f"    ML: {home} {game_odds['home_ml']} / {away} {game_odds['away_ml']}")

        # Model predictions
        if "moneyline" in preds:
            ml = preds["moneyline"]
            lines.append(f"  Moneyline: {ml['pick']} ({ml['confidence']:.0f}% confidence)")
            lines.append(f"    Home win prob: {ml['home_win_prob']:.1%}")

        if "spread" in preds:
            sp = preds["spread"]
            lines.append(f"  Spread: {sp['favored']} by {sp['by_points']} pts (predicted margin: {sp['predicted_margin']:+.1f})")

        if "totals" in preds:
            tot = preds["totals"]
            lines.append(f"  Total: {tot['predicted_total']} pts")

        # Injury adjustments
        if game.get("injury_spread_adj"):
            lines.append(f"  Injury spread adjustment: {game['injury_spread_adj']:+.1f} pts")
        if game.get("injury_total_adj"):
            lines.append(f"  Injury total adjustment: {game['injury_total_adj']:+.1f} pts")
